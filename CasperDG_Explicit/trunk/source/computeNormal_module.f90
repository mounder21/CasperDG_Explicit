Module computeNormal_module
    use my_kinddefs
    use inputs_module
    use Globals_module, only: nodeList,triList,numTri,edgeList,numEulerVars,numEdges
    implicit none
...
Subroutine computeNormals(edgeMap_dPhi,numEdgeGaussPts)
    implicit none
    real(dp)   ,intent(in)  :: edgeMap_dPhi(:,:,:,:)        !(tm,numGPs,xi/eta,face*orints)
    ...
    do edge_id = 1,numEdges 
        ...
        if (triList(leftTri)%straight .eqv. .true.) then
            ...
            print*,'cant computed curved yet,need a_x and a_y,computeNormal' 
        end if 
...
Subroutine allocate_normals(numEdgeGaussPts)
    ...
    allocate(NormalLengths(3,numEdgeGaussPts,numEdges))
