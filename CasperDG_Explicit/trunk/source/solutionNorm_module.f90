...
Subroutine Lp_solution_norm(solCoeffs,numGaussPts,p,norm)
    ...
    use norm_module
    use Globals_module, only: numTri,triList,numEulerVars
    use initializeBasis_module, only: Phi,solutionPhi
    use inputs_module, only: totalModes
    implicit none
    
    real(dp),intent(in) :: solCoeffs(:,:,:),p
    integer(i4),intent(in) :: numGaussPts
    real(dp),intent(out)   :: norm(numEulerVars)
    
    real(dp) :: ax(3),ay(3),x(numGaussPts),y(numGaussPts),res(numGaussPts),f,tempNorm(numEulerVars,numTri)
    real(dp) :: q(numEulerVars,numGaussPts),vec,oneOverp
    ...
    do elem_id = 1, numTri
        ...
        Call projectCell(solCoeffs(:,:,elem_id),Phi,q) !q(numEulerVars,numGaussPts)
        do j = 1, numEulerVars 
            do k = 1, numGaussPts
