Module get_delta_t_module
    ...
    use Globals_module, only: numFields,numTri
...
Subroutine get_delta_t(solCoeffs)
    ...
    real(dp)    :: locQ(numFields,numGaussPts),maxU,maxV,maxC,pOverRho(numGaussPts),c(numGaussPts)
    ...
    do elem_id = 1, numTri
        Call projectCell(solCoeffs(:,:,elem_id),Phi,locQ) ! locQ(numFields,npgs)
        maxU = 0._dp
        maxV = 0._dp
        maxC = 0._dp
        do GP = 1,numGaussPts
