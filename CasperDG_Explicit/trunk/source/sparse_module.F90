module sparse_module
    ...
    type(sparse_struct), pointer :: sparse(:,:)     ! array of cell struct
	...
	subroutine allocate_sparse
            use mesh_module, only : ncell
            implicit none

            integer(i4) :: istat

            allocate(sparse(ncell,ncell),stat = istat)
	...
	subroutine allocate_sparse_block(i,j,nrow,ncol,pivot)
		
		implicit none
		integer(i4),intent(in) :: i,j,nrow,ncol,pivot

		integer(i4) :: istat,n
		
		sparse(i,j)%nrow = nrow
		sparse(i,j)%ncol = ncol
		sparse(i,j)%nonzero = 1
		
		allocate(sparse(i,j)%value(nrow*ncol),stat = istat)		
		
		sparse_memory = sparse_memory + 8.0_dp*real(nrow*ncol,dp)/1024.0_dp/1024.0_dp
	...
	subroutine deallocate_sparse_block(i,j)
            ...
            sparse_memory = sparse_memory - 8.0_dp*real(sparse(i,j)%nrow*sparse(i,j)%ncol,dp)/1024.0_dp/1024.0_dp

            deallocate(sparse(i,j)%value)
