  ...
  subroutine plotSolution(project,solCoeffs)
    ...
    if( io /= 0) then
       ...
       do elem_id = 1, numTri
         ...
         Call projectCell(solCoeffs(:,:,elem_id),solutionPhi,locQ) ! locQ(numFields,3 nodes per tri)
  ...
  subroutine plotSolution6pt(project,solCoeffs)
    ...
    if( io /= 0) then
       ...
       do elem_id = 1, numTri
         ...
         Call projectCell(solCoeffs(:,:,elem_id),solutionPhi,locQ) ! locQ(numFields,6 nodes per tri)
         rho(1) = locQ(1,1)
         rho(2) = locQ(1,2)
         rho(3) = locQ(1,3)
         rho(4) = locQ(1,4)
...
!         plot rho
  ...
  subroutine plotResidual(project,res)
    ...
    character(*),intent(in)                      :: project
    real(dp),    intent(in)                      :: res(:,:,:) !res(numFields,totalModes,numTri)
    ...
    if( io /= 0) then
       ...
       do elem_id = 1, numTri
         Call projectCell(res(:,:,elem_id),solutionPhi,locQ) ! locQ(numFields,3 nodes per tri)
...
!       
  ...
  Subroutine write_Solution_Coefficients(project,solCoeffs)
    ...
    character(*),intent(in)                      :: project
    real(dp),    intent(in)                      :: solCoeffs(:,:,:) !res(numFields,totalModes,numTri)
